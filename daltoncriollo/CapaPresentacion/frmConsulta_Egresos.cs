...
{
    public partial class frmConsulta_Egresos : Form
    {
        public frmConsulta_Egresos()
        {
            InitializeComponent();
        }
        ...
        private void Buscar()
        {
            try
            {
                dgvResumen.DataSource = N_Egreso.ConsultarFechas(Convert.ToDateTime(ddesde.Value), Convert.ToDateTime(dhasta.Value));
                this.formato();
            }
            catch (Exception ex)
            {
        ...
        private void frmConsulta_Egresos_Load(object sender, EventArgs e)
        {
